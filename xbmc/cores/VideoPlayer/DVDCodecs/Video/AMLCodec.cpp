...
bool CAMLCodec::GetPicture(DVDVideoPicture *pDvdVideoPicture)
{
  ...
  //AML video is in general 1 frame to fast - sync audio
  ...
  while (!m_bStop)
  {
...

    {
      ...
      if (DequeueBuffer(pts) == 0)
      {
        ...
        if (g_advancedSettings.CanLogComponent(LOGVIDEO))
          CLog::Log(LOGDEBUG, "CAMLCodec::Process: omx:%d pts:%0.4f", (int)pts, static_cast<float>(pts + m_start_pts)/PTS_FREQ);
        m_ready_event.Set();
      }
    }
  }
